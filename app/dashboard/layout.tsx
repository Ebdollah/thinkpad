...
const DashboardLayout = ({ children }: { children: ReactNode }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white">
            ...
            {/* Tabs - Responsive */}
            <nav
                className={`${menuOpen ? "block" : "hidden sm:flex"
            ...
            >
                {tabs.map((tab) => (
                    <Link
                        key={tab.href}
                        href={tab.href}
                        className={`px-6 py-2 text-center rounded-md transition-all duration-200 text-lg font-semibold ${pathname === tab.href ? "bg-amber-600 text-white" : "text-gray-400 hover:text-white"
                            }`}
                    >
                        {tab.name}
                    </Link>
